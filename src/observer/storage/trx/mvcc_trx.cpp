...
RC MvccTrx::update_record(Table *table,const FieldMeta *fieldmeta, const Value *values,Record &record)
{
  // Field begin_field;
  // Field end_field;
  // trx_fields(table, begin_field, end_field);
  ...
  // end_field.set_int(record, -trx_id_);
  // RC rc = log_manager_->append_log(CLogType::UPDATE, trx_id_, table->table_id(), record.rid(), 0, 0, nullptr);
