...
RC Table::destroy(const char *path, const char *name, const char *base_dir)
{
  ...
  if (!(fd < 0)&&(EEXIST == errno)){
    LOG_ERROR("Failed to drop table file, it has been created. %s, EEXIST, %s", path, strerror(errno));
  ...
  if(remove(data_file.c_str()) != 0)  {
    LOG_ERROR("Failed to remove file %s, due to %s.", data_file.c_str(), strerror(errno));
